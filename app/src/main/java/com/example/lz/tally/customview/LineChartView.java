...
public class LineChartView extends View {
    ...
    //背景
    private String backColor = "#01786B";
    //文字大小
    ...
    private void init() {
        ...
        mBackgroundPaint.setColor(Color.parseColor(backColor));
    ...
    private void drawBackground(Canvas canvas){
        Rect rect = new Rect(0,0,widthSpecSize,heightSpecSize);
    ...
    private void drawData(Canvas canvas) {
        ...
        try {
            if (mDateList != null && mEntityList != null && mEntityList.size() != 0) {
                ...
                long endTime = sdf.parse(mDateList.get(6).split(" ")[0] + " 23:59:59").getTime();
                long startTime = sdf.parse(mDateList.get(0).split(" ")[0] + " 00:00:00").getTime();

                float l = (float) ((width )) / (mDateList.size());
                ...
                for (int i = 0; i < mEntityList.size(); i++) {
                    ...
                    float x = ((float) 2*i+1)/2*l+getPaddingLeft();
...
//                    mTextPaint);
            ...
            if (i == mList.size() - 1) {
                ...
                List<String> list = DateUtils.getListWeekOfDate(new Date());
...
//                            mTextPaint);
                    float x =0,y = 0;
//                    x = getPaddingLeft() + getTextWidth(list.get(j), mTextPaint) * 2 + v * j + v / 2 - (float) getTextWidth(mDateList.get(0).substring(8, 10), mTextPaint) / 2;
                    y = (float) getTextHeight(mDateList.get(0), mTextPaint) * 2 + getTextHeight(normalText, mTextPaint) * 2 + getPaddingTop() + h * i;

                    x = mPointentities.get(j).getX()-getTextWidth(list.get(j), mTextPaint)/2;


                    canvas.drawText(list.get(j),
                            x,
                            y,
                            mTextPaint);
                }
